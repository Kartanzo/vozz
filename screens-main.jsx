// ============================================================
// 3LACKD CRM — Dashboard + Pipeline screens
// ============================================================
...
const Pipeline = ({ t, lang, onOpenAccount, density }) => {
 ...
 const [, forceTickLocks] = React.useState(0);
 const [locks, setLocks] = React.useState(() => (typeof window.loadLocks === "function") ? window.loadLocks() : []);
 // Re-renderiza a cada 60s para expirar fidelizações
 React.useEffect(() => {
 const id = setInterval(() => forceTickLocks(x => x + 1), 60000);
 return () => clearInterval(id);
 }, []);
 // Semeia locks dummy na primeira vez que abre o pipeline
 React.useEffect(() => {
 if (locks.length === 0 && window.CLASSIFICACAO_GRUPOS) {
 const now = Date.now(); const H = 3600000, D = 24 * H;
 const seed = [
 { accountId: "a13", vendorId: "u1", grupoId: "sem-contato", subgrupo: "Não responde", lockedAt: now - 6*H, expiresAt: now + 42*H, renovacoes: 0 },
 { accountId: "a14", vendorId: "u2", grupoId: "conversa-iniciada", subgrupo: "Enviado Catálogo por e-mail/whatsapp", lockedAt: now - 2*D, expiresAt: now + 5*D, renovacoes: 0 },
 { accountId: "a15", vendorId: "u3", grupoId: "negociacao", subgrupo: "Enviado Cotação para o cliente", lockedAt: now - 2*D, expiresAt: now + 5*D, renovacoes: 0 },
 { accountId: "a16", vendorId: "u4", grupoId: "excluir", subgrupo: "Pediu para não contactar novamente", lockedAt: now - 30*D, expiresAt: -1, renovacoes: 0 },
 { accountId: "a17", vendorId: "u4", grupoId: "conversa-iniciada", subgrupo: "Cliente fazendo levantamento de estoque", lockedAt: now - 10*D, expiresAt: now + 12*D, renovacoes: 1 },
 ];
 setLocks(seed);
 if (typeof window.saveLocks === "function") window.saveLocks(seed);
 }
 // eslint-disable-next-line
 }, []);

 const updateLocks = (next) => { setLocks(next); if (window.saveLocks) window.saveLocks(next); };
 const classificarConta = async (grupoId) => {
 const GRUPOS = window.CLASSIFICACAO_GRUPOS || [];
 const grupo = GRUPOS.find(g => g.id === grupoId);
 if (!grupo) return;
 // Escolhe conta livre
 const livres = ACCOUNTS.filter(a => !window.getActiveLock(locks, a.id));
 if (!livres.length) { blackdAlert("Não há contas livres na carteira.", "Carteira sem contas livres"); return; }
 const opcoes = livres.map((a, i) => `${i + 1}. ${a.name} (${a.city})`).join("\n");
 const idx = await blackdPrompt(`Digite o número da conta:\n\n${opcoes}`, "1", `Classificar em "${grupo.titulo}"`);
 const acc = livres[parseInt(idx, 10) - 1];
 if (!acc) return;
 const subOpts = grupo.subgrupos.map((s, i) => `${i + 1}. ${s}`).join("\n");
 const sIdx = await blackdPrompt(`Subgrupo para ${acc.name}:\n\n${subOpts}`, "1", "Selecionar subgrupo");
 const sub = grupo.subgrupos[parseInt(sIdx, 10) - 1];
 if (!sub) return;
 const now = Date.now();
 const expiresAt = grupo.fidelizacaoHoras === -1 ? -1 : now + grupo.fidelizacaoHoras * 3600000;
 const next = [...locks.filter(l => l.accountId !== acc.id), {
 accountId: acc.id, vendorId: "u1", grupoId, subgrupo: sub,
 lockedAt: now, expiresAt, renovacoes: 0,
 }];
 updateLocks(next);
 };
 const renovarLock = (lock) => {
 const grupo = (window.CLASSIFICACAO_GRUPOS || []).find(g => g.id === lock.grupoId);
 if (!grupo?.renovavel) { blackdAlert("Este tipo não pode ser renovado.", "Não renovável"); return; }
 ...
 updateLocks(locks.map(l => l === lock ? { ...l, expiresAt: Date.now() + grupo.renovacaoDias * 24 * 3600000, renovacoes: l.renovacoes + 1 } : l));
 };
 const liberarLock = async (lock) => {
 if (!(await blackdConfirm("Liberar este cliente da carteira?", "Liberar"))) return;
 updateLocks(locks.filter(l => l !== lock));
 };

 const addColumn = async () => {
 const name = await blackdPrompt("Como vai se chamar a nova coluna do pipeline?", "", "Nova coluna");
 if (!name) return;
 ...
 if (!(await blackdConfirm("Os deals desta coluna voltarão para a 1ª coluna. Continuar?", "Remover coluna"))) return;
 ...
 const name = await blackdPrompt("Qual o nome do novo negócio?", "", "Novo deal");
 if (!name) return;
 ...
 <button aria-current={filter === "classificacao"} onClick={() => setFilter("classificacao")} title="Mapa de Classificação de Contatos · Fidelização por Contatos">
 Classificação
 </button>
 </div>
 ...
 {(window.CLASSIFICACAO_GRUPOS || []).map(grupo => {
 const grupoLocks = locks.filter(l => l.grupoId === grupo.id && (l.expiresAt === -1 || l.expiresAt > Date.now()));
 return (
 <div key={grupo.id} className="kanban-col">
 <div className="kanban-col-header">
 <span className="swatch" style={{ background: grupo.cor }}/>
 <span className="kanban-col-title" style={{ color: grupo.cor }}>{grupo.titulo}</span>
 <span className="kanban-col-count">{grupoLocks.length}</span>
 <span className="kanban-col-sum" style={{ fontSize: 10 }}>{grupo.fidelizacaoLabel}</span>
 </div>
 <div className="kanban-body">
 <div className="text-3" style={{ fontSize: 11, padding: "4px 8px 8px", lineHeight: 1.4 }}>{grupo.desc}</div>
 {grupoLocks.map((lock, i) => {
 const acc = ACCOUNTS.find(a => a.id === lock.accountId);
 const owner = REPS.find(r => r.id === lock.vendorId);
 if (!acc) return null;
 const restante = window.fmtRestante ? window.fmtRestante(lock.expiresAt) : "";
 return (
 <div key={i} className="kanban-card" onClick={() => onOpenAccount(acc.id)} style={{ borderLeft: `3px solid ${grupo.cor}` }}>
 <div className="row" style={{ gap: 4, marginBottom: 6 }}>
 <span className="badge" style={{ background: grupo.cor + "22", color: grupo.cor, fontSize: 10, fontWeight: 700, padding: "2px 6px" }}>
 <Icon name="lock" size={9}/> {restante}
 </span>
 {grupo.renovavel && lock.renovacoes > 0 && (
 <span className="badge" style={{ fontSize: 10, padding: "2px 6px" }}>Renov. {lock.renovacoes}/{grupo.renovacoesMax}</span>
 )}
 </div>
 <div className="kanban-card-co">{acc.name}</div>
 <div className="kanban-card-deal">{lock.subgrupo}</div>
 <div className="kanban-card-meta">
 <span className="text-3" style={{ fontSize: 11 }}>{acc.city}</span>
 </div>
 <div className="kanban-card-foot">
 {owner && <Avatar user={owner} size={20}/>}
 <span className="text-3" style={{ fontSize: 11.5 }}>{owner?.name?.split(" ")[0]}</span>
 <span style={{ marginLeft: "auto", display: "flex", gap: 4 }} onClick={(e) => e.stopPropagation()}>
 {grupo.renovavel && lock.renovacoes < grupo.renovacoesMax && lock.expiresAt !== -1 && (
 <button className="btn btn-sm" style={{ padding: "2px 6px", fontSize: 10 }} onClick={() => renovarLock(lock)}>Renovar</button>
 )}
 <button className="btn btn-sm btn-ghost" style={{ padding: "2px 6px", fontSize: 10 }} onClick={() => liberarLock(lock)}>Liberar</button>
 </span>
 </div>
 </div>
 );
 })}
 <button className="kanban-add" onClick={() => classificarConta(grupo.id)}>+ Classificar cliente</button>
 </div>
 </div>
 );
 })}
 ...
 {(() => {
 const lock = window.getActiveLock && window.getActiveLock(locks, acc.id);
 if (!lock) return null;
 const grupo = (window.CLASSIFICACAO_GRUPOS || []).find(g => g.id === lock.grupoId);
 ...
 <span className="badge" title={`Fidelizado — ${grupo.titulo} · ${lock.subgrupo} · ${restante}`}
 style={{ background: grupo.cor + "22", color: grupo.cor, fontSize: 10, fontWeight: 700, padding: "2px 6px" }}>
 <Icon name="lock" size={9}/> {restante}
 </span>
 </div>
 );
