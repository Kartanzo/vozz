// ============================================================
// 3LACKD CRM — Catálogo de produtos / Ficha técnica / Gerador PDF
// ============================================================

// Persistência de imagens no localStorage (base64)
const IMG_KEY = "blackd_product_images";
...
const Catalog = ({ t, lang }) => {
 ...
 <button className="btn btn-sm" onClick={() => blackdAlert("Em breve: cadastro de novos produtos. Por enquanto, edite data.js.", "Novo produto")}><Icon name="plus" size={13}/> Novo produto</button>
...
const ProductDetailModal = ({ product, onClose, selected, onToggle }) => {
 ...
 const email = await blackdPrompt("Para qual e-mail enviar a ficha técnica?", "", "Enviar ficha por email");
...
const CatalogPdfModal = ({ products, onClose }) => {
 ...
 <div className="flex-1">
 <div className="modal-title">Catálogo 3lackd — preview</div>
 <div className="text-3" style={{ fontSize: 12 }}>{products.length + 1} páginas · A4 · estilo 3lackd</div>
 </div>
 <button className="icon-btn" onClick={onClose}><Icon name="x" size={16}/></button>
 </div>
 <div className="modal-body">
