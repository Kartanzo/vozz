...
const Quote = ({ t, lang }) => {
  ...
  const saveQuote = () => {
    if (!items.length) { vozzAlert("Adicione pelo menos um produto antes de salvar.", "Pedido vazio"); return; }
    const q = {
      id: "q_" + Date.now(),
    ...
    vozzAlert("Pedido simulado salvo! Veja na aba 'Histórico'.", "✅ Salvo");
          ...
          <div className="card-header">
            ...
            {quotes.length > 0 && (
              <button className="btn btn-sm" style={{ marginLeft: "auto" }}
                      onClick={async () => { if (await vozzConfirm("Apagar todo o histórico de simulações?", "Limpar histórico")) { setQuotes([]); saveQuotes([]); } }}>
