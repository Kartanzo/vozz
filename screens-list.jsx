// ============================================================
// 3LACKD CRM — Accounts list, Account Detail, Schedule
// ============================================================
...
const Accounts = ({ t, lang, onOpenAccount }) => {
 ...
 const locks = (typeof window.loadLocks === "function") ? window.loadLocks() : [];
 const CURRENT_USER = "u1"; // Camila Reis (mock)
 ...
 const link = document.createElement("a"); link.href = url; link.download = "3lackd_contas.csv"; link.click();
 URL.revokeObjectURL(url);
 }}><Icon name="export" size={13}/> {t.export}</button>
 <button className="btn btn-sm btn-primary" onClick={async () => {
 const name = await blackdPrompt("Nome da empresa ou contato:", "", "Novo lead");
 if (name) blackdAlert(`Lead "${name}" criado (mock). Em produção seria salvo no CRM.`, " Criado");
 ...
 <td>
 {(() => {
 const lock = window.getActiveLock && window.getActiveLock(locks, a.id);
 if (lock) {
 ...
 const mine = lock.vendorId === CURRENT_USER;
 return (
 <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
 <span className="badge" title={`Em atendimento por ${owner?.name} · ${lock.subgrupo} · ${restante}${mine ? "" : " · bloqueado para outros vendedores"}`}
 style={{ background: (grupo?.cor || "#888") + "22", color: grupo?.cor, fontWeight: 700 }}>
 <Icon name="lock" size={10}/> {mine ? "Em atendimento" : "Em atendimento (outro vendedor)"}
 </span>
 <span className="text-3" style={{ fontSize: 10.5 }}>{owner?.name?.split(" ")[0]} · {restante}</span>
 </div>
 );
 }
 return a.inactive ? <span className="badge badge-danger"><span className="dot"/> Inativo</span>
...
const AccountDetail = ({ accountId, t, lang, onBack, onMessage }) => {
 ...
 {(() => {
 const lock = window.getActiveLock && window.getActiveLock(window.loadLocks ? window.loadLocks() : [], acc.id);
 if (!lock) return null;
 const grupo = (window.CLASSIFICACAO_GRUPOS || []).find(g => g.id === lock.grupoId);
 const lockOwner = REPS.find(r => r.id === lock.vendorId);
 const restante = window.fmtRestante ? window.fmtRestante(lock.expiresAt) : "";
 const mine = lock.vendorId === "u1";
 return (
 <span className="badge" style={{ background: (grupo?.cor || "#888") + "22", color: grupo?.cor, fontWeight: 700 }}
 title={`Em atendimento por ${lockOwner?.name} · ${lock.subgrupo}${mine ? "" : " · bloqueado para outros vendedores"}`}>
 <Icon name="lock" size={11}/> {mine ? "Em meu atendimento" : `Em atendimento por ${lockOwner?.name?.split(" ")[0]}`} · {restante}
 </span>
 );
 })()}
 </div>
 {(() => {
 const lock = window.getActiveLock && window.getActiveLock(window.loadLocks ? window.loadLocks() : [], acc.id);
 if (!lock || lock.vendorId === "u1") return null;
 const grupo = (window.CLASSIFICACAO_GRUPOS || []).find(g => g.id === lock.grupoId);
 const lockOwner = REPS.find(r => r.id === lock.vendorId);
 return (
 <div style={{
 padding: "8px 12px", borderRadius: 8, marginBottom: 8,
 background: (grupo?.cor || "#888") + "15",
 borderLeft: `3px solid ${grupo?.cor || "#888"}`,
 fontSize: 12.5, color: "var(--text-1)",
 }}>
 <Icon name="lock" size={12}/> <b>Cliente em fidelização</b> — atendido por <b>{lockOwner?.name}</b> ({lock.subgrupo}). Contato bloqueado para outros vendedores até {window.fmtRestante ? window.fmtRestante(lock.expiresAt) : "expirar"}.
 </div>
 );
 })()}
...
const DetailStakeholders = ({ acc }) => {
 ...
 onClick={() => blackdAlert("Em breve: cadastrar stakeholders manualmente.", "Adicionar stakeholder")}>
 ...
 <td>
 <button className="btn btn-sm" onClick={() => blackdAlert(`Convite enviado para ${s.nome} (${s.cargo}).`, " Reunião agendada")}>
